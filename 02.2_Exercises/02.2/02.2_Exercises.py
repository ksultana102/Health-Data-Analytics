...
rs = db.executeQuery(..., engineDetails)
rs.to_csv(..., index=False, header=True)


##### WRITE: TSV ##################################################################################
# %%
# (1) Write the same results (reuse) to patients_pd.tsv
rs.to_csv(..., index=..., header=..., sep=...)
...
st = time.time()
covid.to_sql(..., db.engine(...), schema=..., if_exists=..., index=..., method=...)
