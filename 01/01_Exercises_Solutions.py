...
st = time.time()
db.execute('CREATE TABLE test.patients (LIKE armc_1m.patients INCLUDING ALL)', engineDetails)
...
df.to_sql('patients', engine, schema='test')
print('Time:', time.time()-st)
...
st = time.time()
df = db.executeQuery('SELECT * FROM armc_1m.patients ORDER BY patient_id LIMIT 100000', engine)
df.to_sql('patients', engine, schema='test', if_exists='append', index=False)
print('Time:', time.time()-st)
